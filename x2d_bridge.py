...
def _format_design_hits(hits: list, header: str = ""):
    ...
    for h in hits:
        ...
        did = str(d.get("id") or d.get("designId") or "")
        title = str(d.get("title") or d.get("designTitle") or "")
        creator = (d.get("designCreator") or {}).get("name", "") or d.get("creatorName", "")
        likes = int(d.get("likeCount") or d.get("likes") or 0)
        downloads = int(d.get("downloadCount") or d.get("downloads") or 0)
        print(f"  {did:<8} likes={likes:<5} dls={downloads:<6}  by {creator[:18]:<18}  {title[:50]}")
...
def cmd_cloud_search(args: argparse.Namespace) -> int:
    ...
    if cli.session.empty:
        print("not logged in", file=sys.stderr); return 1
    try:
        r = cli.search_designs(args.query, limit=int(args.limit), offset=int(args.offset))
    except cloud_client.CloudError as e:
        print(f"cloud API failed: {e}", file=sys.stderr); return 1
    if args.json:
        print(json.dumps(r, indent=2, default=str)); return 0
    _format_design_hits(r.get("hits") or [],
                        header=f"{r.get('total', 0)} match(es) for {args.query!r}:")
    return 0
...
def cmd_cloud_browse(args: argparse.Namespace) -> int:
    ...
    if cli.session.empty:
        print("not logged in", file=sys.stderr); return 1
    try:
        r = cli.browse_designs_by_nav(args.nav, limit=int(args.limit), offset=int(args.offset))
    except cloud_client.CloudError as e:
        print(f"cloud API failed: {e}", file=sys.stderr); return 1
    if args.json:
        print(json.dumps(r, indent=2, default=str)); return 0
    _format_design_hits(r.get("hits") or [],
                        header=f"{r.get('total', 0)} designs in nav={args.nav!r}:")
    return 0
...
def cmd_cloud_design(args: argparse.Namespace) -> int:
    ...
    print(f"Stats        : likes={r.get('likeCount',0)}  dls={r.get('downloadCount',0)}  "
          f"collections={r.get('collectionCount',0)}  comments={r.get('commentCount',0)}")
    instances = r.get("instances") or []
    print(f"Instances    : {len(instances)}")
    for inst in instances[:5]:
        print(f"  - id={inst.get('id')} title={inst.get('title','')!r:<40} "
...
def cmd_cloud_design_remixes(args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(json.dumps(r, indent=2, default=str)); return 0
    _format_design_hits(r.get("hits") or [],
                        header=f"{r.get('total',0)} remix(es) of design {args.design_id}:")
    return 0
...
def cmd_cloud_presets(args: argparse.Namespace) -> int:
    ...
    if cli.session.empty:
        print("not logged in", file=sys.stderr); return 1
    try:
        r = cli.get_slicer_presets(version=args.version, public=bool(args.public))
    except cloud_client.CloudError as e:
        print(f"cloud API failed: {e}", file=sys.stderr); return 1
    if args.json:
        print(json.dumps(r, indent=2, default=str)); return 0
...
def main() -> int:
    ...
    cli_browse = sub.add_parser(
        "cloud-browse",
        help="Browse MakerWorld designs by nav key (Trending / Foryou / Household / etc.).")
    cli_browse.add_argument("nav", help="nav key (use `cloud-search-suggest` "
                                        "or `/v1/search-service/homepage/nav` to discover)")
    ...
    cli_design = sub.add_parser(
        "cloud-design",
        help="Show full record for a MakerWorld design ID (title, creator, instances, "
             "like/download counts, signed S3 URL to its .3mf bundle).")
    ...
    cli_pre = sub.add_parser(
        "cloud-presets",
        help="User's cloud-synced slicer presets (print / filament / printer profiles).")
    ...
    cli_pre.add_argument("--public", action="store_true",
                         help="Include Bambu's shipped public presets as well")
