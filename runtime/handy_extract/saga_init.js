...
function installAntiAntiDebug() {
  ...
  // Synchronous chain: nested setTimeouts silently never fire in this
  // gadget script-mode runtime. Install crypto hooks inline here. Modules
  // that aren't loaded yet are skipped by tryHook (findExportByName → null)
  // — late-binding libcrypto.so instances are still picked up because each
  // dlopen triggers our existing Module-enumerate code paths.
  try {
    if (typeof _fcap === 'function') _fcap('[chain] sync crypto-installer entry');
    let total = 0;
    if (typeof hookOpenSSL === 'function')          total += (hookOpenSSL()          || 0);
    if (typeof hookMbedTLS === 'function')          total += (hookMbedTLS()          || 0);
    if (typeof hookFlutterBoringSSL === 'function') total += (hookFlutterBoringSSL() || 0);
    if (typeof hookHTTPS === 'function')            hookHTTPS();
    if (typeof _fcap === 'function') _fcap('[chain] installed ' + total + ' crypto hooks');
  } catch (e) {
    if (typeof _fcap === 'function') _fcap('[chain] sync crypto-installer threw: ' + e + ' stack=' + (e && e.stack));
  }

  // Initialize _hookedAddrs even though we no longer hook dlopen — keeps
  // tryHook's idempotency guard happy in case the hook fns get re-run from
  // some other path (e.g. a future setInterval).
  if (typeof globalThis._hookedAddrs === 'undefined')
    globalThis._hookedAddrs = new Set();

  // NOTE 2026-05-20: Previously installed an Interceptor on android_dlopen_ext
  // to rescan for libflutter.so when the packer late-loads it. That triggered
  // SHIELD's tamper-detect (poison-jump to 0xdead5014 ~50ms after init.js
  // completes). Removed. To pick up libflutter.so we now rely on a polling
  // setInterval which avoids modifying libdl.so code bytes.
  try {
    let _rescanCount = 0;
    const _maxRescans = 60;  // 60 * 1s = 60s window
    const _iv = setInterval(() => {
      _rescanCount++;
      try {
        const extra = (hookOpenSSL() || 0);
        if (extra > 0 && typeof _fcap === 'function')
          _fcap('[chain] poll rescan #' + _rescanCount + ' added ' + extra + ' hooks');
      } catch (e) {}
      if (_rescanCount >= _maxRescans) clearInterval(_iv);
    }, 1000);
    if (typeof _fcap === 'function') _fcap('[chain] poll-based rescan armed (1s × 60)');
  } catch (e) {
    if (typeof _fcap === 'function') _fcap('[chain] poll rescan install failed: ' + e);
  }
}
...
function hookOpenSSL() {
  ...
  function tryHook(mod, sym, mkHandler) {
    ...
    if (globalThis._hookedAddrs && globalThis._hookedAddrs.has(key)) return false;
    try {
      Interceptor.attach(addr, mkHandler(mod.name, sym));
      if (globalThis._hookedAddrs) globalThis._hookedAddrs.add(key);
